...
exports.getAll = function ( resourceName, populate, options ) {
  populate   = populate || '';
  userDomain = ( options.useUserDomain ) ? options.userDomain : false;
  ...
  return function ( req, res, next ) {
    ...
    if ( userDomain ) {
      if ( !req.auth.user || !req.auth.user._id ) {
        return respond.code.notauthorized(res);
      }

      query[ userDomain ] = req.user._id;
    }

    if( query._distinct === 'true' ) {
      return Model.distinct(query, select).exec().then(function ( items ) {
        res.send( items );
      });
    }
...
exports.getById = function ( resourceName, populate, options ) {
  populate   = populate || '';
  userDomain = ( options.useUserDomain ) ? options.userDomain : false;
  ...
  return function ( req, res, next ) {
    ...
    .exec(function ( err, record ) {
      ...
      if ( userDomain && record[ userDomain ].toString() !== req.auth.user._id.toString() ) {
        return respond.res.notauthorized(res);
      }
